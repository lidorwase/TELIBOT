...
CHANNEL_USERNAME = "@dealsumustcheck"  # עדכן במידת הצורך
USER_AGENT = "Mozilla/5.0 (Windows NT 10.0; Win64; x64)"

bot = telebot.TeleBot(BOT_TOKEN, parse_mode="HTML")

# זיכרון קצר-טווח: user_id -> info שנשלף בשלב 1
ANALYSES = {}

# === שרת קטן לשמירה בחיים (Railway) ===
app = Flask(__name__)
@app.get("/")
...
def detect_currency(txt):
    ...
    map_ = {"₪":"ILS","$":"USD","US$":"USD","USD":"USD","€":"EUR","EUR":"EUR","£":"GBP","GBP":"GBP","руб":"RUB","AED":"AED","SAR":"SAR","₹":"INR"}
    t = str(txt)
                ...
                if isinstance(v, list) and v:
                    ...
                    if isinstance(first, dict):
                        for sub in ("value","amount","display","text","price"):
                            if sub in first and isinstance(first[sub], (str,int,float)): return first[sub]
                    else:
                        return first
        ...
        if m:
            try: return json.loads(m.group(1))
                ...
                try:
                    ...
                    return json.loads(m.group(1)[:end+1])
...
def resolve_url(url: str) -> str:
    ...
    try:
        ...
        final_url = url
    # אחידות: aliexpress.us -> .com
    try:
        pu = urllib.parse.urlparse(final_url)
        if pu.netloc.lower().startswith("aliexpress.us"):
            final_url = urllib.parse.urlunparse(pu._replace(netloc="www.aliexpress.com"))
    except Exception:
        pass
    return final_url
...
def pull_product_data(any_url: str):
    """
    מקבל כל קישור (קצר/ארוך), שולף title/price/rating/orders/image + קישור מוצר סופי ובנוסף product_id.
    לא מחזיר קישור אפיליאייט – נשאיר את המקורי לשלב הפוסט.
    """
    ...
    # אם חסר מחיר/דירוג – ננסה מובייל לפי pid
    ...
    info = {
        ...
        "product_url": final_url,  # לא אפיליאייט; רק מידע
    }
    return info
...
def compose_caption(info: dict, outbound_url: str):
    ...
    # כאן חשוב: נשתמש בקישור שאתה נתת /post – קישור אפיליאייט שלך
    lines.append(f'🔗 <a href="{html.escape(outbound_url)}">לקנייה באלי אקספרס</a>')
    return "\n".join(lines)

# ===== שלב 1: /analyze <קישור מוצר> =====
@bot.message_handler(commands=["analyze"])
def cmd_analyze(m):
    parts = m.text.split(maxsplit=1)
    if len(parts) < 2:
        bot.reply_to(m, "שלח כך: /analyze <קישור מוצר מאלי אקספרס>")
        return
    url = parts[1].strip()
    try:
        info = pull_product_data(url)
        ANALYSES[m.from_user.id] = info  # שמירה לפי משתמש
        # תצוגה מקדימה
        ...
        bot.send_message(m.chat.id, "✅ נותח בהצלחה. עכשיו שלח:\n/post <קישור אפיליאייט> כדי לפרסם לערוץ.")
    except Exception as e:
        bot.reply_to(m, f"❌ לא הצלחתי לנתח את המוצר: {e}")

# ===== שלב 2: /post <קישור אפיליאייט> =====
@bot.message_handler(commands=["post"])
def cmd_post(m):
    parts = m.text.split(maxsplit=1)
    if len(parts) < 2:
        bot.reply_to(m, "שלח כך: /post <קישור אפיליאייט>")
        return
    aff_url = parts[1].strip()
    info = ANALYSES.get(m.from_user.id)
    if not info:
        bot.reply_to(m, "🔎 קודם הרץ: /analyze <קישור מוצר>, ואז /post <קישור אפיליאייט>.")
        return
    try:
        caption = compose_caption(info, outbound_url=aff_url)  # כאן יוצא עם קישור האפיליאייט שלך
        if info.get("image"):
        ...
        bot.reply_to(m, "✅ פורסם לערוץ! (עם קישור האפיליאייט ששלחת)")
    except Exception as e:
        bot.reply_to(m, f"❌ שגיאה בפרסום: {e}")

# ניקוי זיכרון ידני אם צריך
@bot.message_handler(commands=["reset"])
def cmd_reset(m):
    ANALYSES.pop(m.from_user.id, None)
    bot.reply_to(m, "🧹 נמחק המידע שנשמר עבורך. אפשר להתחיל מחדש עם /analyze")

# ברירת מחדל: עזרה
@bot.message_handler(commands=["start", "help"])
def start_help(m):
    bot.reply_to(m,
        "היי! שני שלבים פשוטים:\n"
        "1) /analyze <קישור מוצר מאלי אקספרס>\n"
        "2) /post <קישור אפיליאייט> – פרסום לערוץ\n\n"
        "אפשר לנקות עם /reset")

# הפעלה
keep_alive()
print("🤖 Two-step AliExpress bot online")
